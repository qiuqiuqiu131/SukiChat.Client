...
using ChatClient.Avalonia.Controls.FreindGroup;
using ChatClient.Tool.Data.Group;

namespace ChatClient.Avalonia.Controls.GroupGroup;
...
public class GroupSeparateGroupsView : UserControl
{
    ...
    private void RemoveAllControl()
    {
        ...
        {
            if (control is GroupGroupList groupList)
            {
    ...
    private void InitGroupFriendControl(AvaloniaList<GroupGroupDto> groupFriends)
    {
        ...
        {
            var contorl = (GroupGroupList)_dataTemplate.Build(groupFriend)!;
            ...
            else if (args.Action == NotifyCollectionChangedAction.Remove)
            {
                if (args.OldItems != null && args.OldItems.Count != 0)
                {
                    foreach (var oldItem in args.OldItems)
                    {
                        ...
                        {
                            var control = (GroupGroupList)_itemCollection.FirstOrDefault(d =>
                                d is GroupGroupList control && control.DataContext == groupFriend)!;
    ...
    {
        if (sender is GroupGroupList groupList)
        {
            foreach (var items in _itemCollection)
            {
                if (items is GroupGroupList groupItem)
                {
    ...
    public void SelectItem(GroupRelationDto groupRelationDto)
    {
        ...
        var groupFriend = _itemCollection.FirstOrDefault(d =>
            d is Control control && control.DataContext is GroupGroupDto groupGroup &&
            groupGroup.GroupName.Equals(groupRelationDto.Grouping)) as GroupGroupList;

        if (groupFriend == null) return;

        groupFriend.Open();
        groupFriend.SelectItem(groupRelationDto);
    }
