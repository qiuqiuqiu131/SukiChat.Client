...
namespace ChatClient.Avalonia.Controls.FreindGroup;
...
public class SeparateGroupsView : UserControl
{
    ...
    private void RemoveAllControl()
    {
        ...
        {
            if (control is GroupList groupList)
            {
    ...
    private void InitGroupFriendControl(AvaloniaList<GroupFriendDto> groupFriends)
    {
        ...
        {
            var contorl = (GroupList)_dataTemplate.Build(groupFriend)!;
    ...
    {
        // 点击右键
        if (sender is GroupList groupList && e.GetCurrentPoint(groupList).Properties.IsRightButtonPressed)
        {
            // 显示右键菜单
            // 选中当前项
            ...
            else if (args.Action == NotifyCollectionChangedAction.Remove)
            {
                if (args.OldItems != null && args.OldItems.Count != 0)
                {
                    foreach (var oldItem in args.OldItems)
                    {
                        ...
                        {
                            var control = (GroupList)_itemCollection.FirstOrDefault(d =>
                                d is GroupList control && control.DataContext == groupFriend)!;
    ...
    {
        if (sender is GroupList groupList)
        {
            foreach (var items in _itemCollection)
            {
                if (items is GroupList groupItem)
                {
    ...
    public void SelectItem(FriendRelationDto friendRelationDto)
    {
        ...
        var groupFriend = _itemCollection.FirstOrDefault(d =>
            ...
            groupFriend.GroupName.Equals(friendRelationDto.Grouping)) as GroupList;
