...
namespace ChatClient.Avalonia.Controls.MultiFriendGroup;
...
public class MultiSeparateGroupView : UserControl
{
    ...
    private void RemoveAllControl()
    {
        ...
        {
            if (control is MultiGroupList groupList)
            {
    ...
    private void InitGroupFriendControl(AvaloniaList<GroupFriendDto> groupFriends)
    {
        ...
        {
            var control = (MultiGroupList)_dataTemplate.Build(groupFriend)!;
            ...
            else if (args.Action == NotifyCollectionChangedAction.Remove)
            {
                if (args.OldItems != null && args.OldItems.Count != 0)
                {
                    foreach (var oldItem in args.OldItems)
                    {
                        ...
                        {
                            var control = (MultiGroupList)_itemCollection.FirstOrDefault(d =>
    ...
    {
        if (sender is MultiGroupList groupList)
        {
