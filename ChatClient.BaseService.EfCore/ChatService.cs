...
internal class ChatService : Services.BaseService, IChatService
{
        ...
        if (fileTarget == FileTarget.User)
        {
            ...
            var entity =
                await chatPrivateRepository.GetFirstOrDefaultAsync(predicate: d =>
                    d.ChatId == chatId && d.UserId == userId);
            return entity?.TargetPath;
        }
        else
        {
            var charGroupRepository = _unitOfWork.GetRepository<ChatGroupFile>();
            var entity =
                await charGroupRepository.GetFirstOrDefaultAsync(predicate: d =>
                    d.ChatId == chatId && d.UserId == userId);
            return entity?.TargetPath;
        }
    }
    ...
    public async Task<string?> OperateRetractedChatMessage(string userId, int chatId, FileTarget fileTarget)
    {
        if (fileTarget == FileTarget.User)
        {
            ...
            {
                var chatPrivateRepository = _unitOfWork.GetRepository<ChatPrivate>();
                var entity = await chatPrivateRepository.GetFirstOrDefaultAsync(
                    predicate: d => d.ChatId == chatId,
        ...
        else
        {
            ...
            {
                var chatGroupRepository = _unitOfWork.GetRepository<ChatGroup>();
                var entity = await chatGroupRepository.GetFirstOrDefaultAsync(
                    predicate: d => d.ChatId == chatId,
                    disableTracking: false);
                if (entity != null)
                {
